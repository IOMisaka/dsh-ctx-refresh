/**
 * dsh-ctx-refresh — host half. (Rebuilt for the current 0.1.5-rc deployment: the
 * settings/webServer/credentials contracts and agent/inbox/inserted are unchanged;
 * only the schemastery import is now resolved defensively, because a standalone
 * pnpm install into a profile tree may not resolve it from its own node_modules.)
 *
 ...
 * autoSyncIntervalMinutes } (defaults: on, 30 min), installed through the
 * current SettingsProvider.installSection API. Every user message entering
 ...
 *
 * Writes go through path-addressed settings mutations on llm-pi-ai. Settings
 * path ops descend only through plain objects — array indices are not
...
/** Fallback entry for the plugin-owned settings namespace (auto-sync configuration). */
const DEFAULT_CONFIG = { autoSyncEnabled: true, autoSyncIntervalMinutes: 30 }

/**
 * Resolve the `z` schema constructor at runtime. The host loader resolves bare
 * specifiers from this package's own location: inside the platform monorepo tree
 * @deepseek-ai/schemastery resolves directly; a standalone pnpm install into a
 * profile tree (whose node_modules lacks it) must fall back to an ancestor tree
 * that carries the vendored copy. If neither candidate yields a usable z.object,
 * registration degrades to a callable identity validator — the entry base still
 * supplies every default, so behavior is unchanged either way.
 */
async function resolveZ() {
	const tryModule = async (specifier) => {
		const mod = await import(specifier)
		const z = mod.default ?? mod
		return typeof z.object === 'function' ? z : undefined
	}
	try {
		const z = await tryModule('@deepseek-ai/schemastery')
	...
	try {
		...
		for (let hop = 0; hop < 12; hop += 1) {
			const candidate = path.join(dir, 'node_modules', '@deepseek-ai', 'schemastery', 'lib', 'index.mjs')
			if (fs.existsSync(candidate)) {
				try {
					const z = await tryModule(pathToFileURL(candidate).href)
					if (z !== undefined) return z
				} catch {}
			}
...
/**
 * The auto-sync config schema. When no z resolves, the placeholder keeps
 * registration alive — its `toJSON` must still serve the canonical
 * schemastery envelope ({uid, refs}), because the client rehydrates the
 * served schema and validates the value before the bound scope reports
 * 'ready'; an empty `{}` envelope fails that decode and the settings card
 * never renders (the historical missing-card bug).
 */
function buildConfigSchema(z) {
	...
	placeholder.toJSON = () => ({
		...
		refs: {
			...
			3: { type: 'number', meta: { default: 30 } },
			4: { type: 'object', meta: { default: {} }, dict: { autoSyncEnabled: 1, autoSyncIntervalMinutes: 3 } }
		}
	})
	return placeholder
}
...
export function apply(ctx) {
	...
	void (async () => {
		const schema = buildConfigSchema(await resolveZ())
		try {
			ctx.settings.installSection(ctx, 'dsh-ctx-refresh', schema, DEFAULT_CONFIG, {
